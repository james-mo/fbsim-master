...
export function draw_pitch(canvas: OffscreenCanvas) {
  ...
  if (ctx) {
    ...
    const radius = (PitchDimensions.center_circle_radius / venue.dimensions.length) * length;
    ctx.beginPath();
    ...
    let x1 = (PitchDimensions.penalty_area_length / venue.dimensions.length) * length;
    let y1 = (PitchDimensions.penalty_area_width / venue.dimensions.width) * width;
    ctx.beginPath();
    ctx.rect(50, width / 2 - y1 / 2 + 50, x1, y1);
    ctx.stroke();
    // draw white penalty spot
    x1 = (PitchDimensions.penalty_spot_distance / venue.dimensions.length) * length + 50;
    y1 = width / 2 + 50;
    ctx.beginPath();
    ...
    x1 = (PitchDimensions.six_yard_box_length / venue.dimensions.length) * length;
    ...
    x1 = (PitchDimensions.penalty_spot_distance / venue.dimensions.length) * length;
    y1 = width / 2 + 50;

    const pen_spot2 = [(PitchDimensions.penalty_spot_distance / venue.dimensions.length) * length, width / 2 + 50];
    ctx.beginPath();
    ctx.arc(length - x1 + 50, y1, 2, 0, 2 * Math.PI);
    ctx.fill();
    ctx.stroke();

    x1 = (PitchDimensions.penalty_area_length / venue.dimensions.length) * length;
    ...
    // penalty arcs
    const pen_radius = (PitchDimensions.penalty_arc_radius / venue.dimensions.length) * length;
    let x = length - (PitchDimensions.penalty_area_length / venue.dimensions.length) * length + 50;
    ...
    ctx.strokeStyle = 'white';
    ctx.moveTo(x, y1);
    ctx.arcTo(x - meters_to_px(3.65, venue.dimensions.length, length), width / 2 + 50, x, y2, pen_radius);
    ctx.stroke();

    x = (PitchDimensions.penalty_area_length / venue.dimensions.length) * length + 50;
    ctx.beginPath();
    ctx.moveTo(x, y1);
    ctx.arcTo(x + meters_to_px(3.65, venue.dimensions.length, length), width / 2 + 50, x, y2, pen_radius);
    ctx.stroke();
